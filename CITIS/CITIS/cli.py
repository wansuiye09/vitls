...
with 'CITIS [command] [arguments]' from the shell.
"""
...
import shutil
import glob
from CITIS.api import CommunicationError
...
class Command(object):
    def __init__(self, args):
        parser = self.setup()
        parser.prog = "CITIS {0}".format(sys.argv[1])
...
class init(Command):
    ...
    def __call__(self, args):
        ...
        os.symlink(public, target + "/public/CITIS")
...
class status(Command):
    ...
    def verify(self, session):
        ...
        try:
            da = urllib2.urlopen(
                    "{0}/CITIS/verify.html".format(config.localhost))
            da = da.read().strip()
            if da == "1":
                print "OK"
            else:
    ...
    def __call__(self, args):
        ...
        try:
            ...
            if args.disable:
                ...
                for hit in query:
                    if args.offline:
                        print hit.offlineurl(config.localhost + "ann.html")
                    else:
                        hit.publish()
...
class setup(Command):
    ...
    def resetdatabase(self):
        ...
        #    print "\timport config, CITIS.api"
        #    print "\tCITIS.api.server.purge()"
        database.reinstall()
