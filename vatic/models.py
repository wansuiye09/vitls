...
class Video(CITIS.database.Base):
    __tablename__   = "videos"

    id              = Column(Integer, primary_key = True)
    slug            = Column(String(250), index = True)
    uploaderid      = Column(Integer, ForeignKey(Worker.id))
    upload          = relationship(Worker, backref=backref("videos",
                                                cascade="all,delete"))
    ...
    videoannodocument=Column(String(250))
...
class Label(CITIS.database.Base):
...
class Attribute(CITIS.database.Base):
...
class Segment(CITIS.database.Base):
...
class Job(CITIS.models.HIT):
    __tablename__ = "jobs"
    __mapper_args__ = {"polymorphic_identity": "jobs"}

    id             = Column(Integer, ForeignKey(CITIS.models.HIT.id),
                            primary_key = True)
...
class Path(CITIS.database.Base):
    __tablename__ = "paths"
...
class AttributeAnnotation(CITIS.database.Base):
...
class CompletionBonus(CITIS.models.BonusSchedule):
